...
func (k *kafkaConn) consumer() {
	kafksVersion := sarama.DefaultVersion.String()
	assignor := "range"
	oldest := false
	verbose := false
	keepRunning := true
	log.Printf("Starting a new Sarama consumer:%s\n", k.cluster)

	if verbose {
		sarama.Logger = log.New(os.Stdout, "[sarama] ", log.LstdFlags)
	}

	version, err := sarama.ParseKafkaVersion(kafksVersion)
	if err != nil {
